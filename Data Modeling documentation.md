...
The next step involved implementing login and logout functionality for the system.

### Step 9.1: Login Page Setup

- Created a `login.html` template inside the `templates` folder.
- Used Django’s built-in `LoginView` for handling user authentication.
- Configured URL patterns for login.

### Step 9.2: Redirect Users After Login

- Configured Django to redirect users to the home page after login instead of the default `/accounts/profile/`.
